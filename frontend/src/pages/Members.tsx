...
export default function Members() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B1120] text-slate-900 dark:text-slate-100 font-sans flex flex-col">
      <nav className="border-b border-slate-200 dark:border-[#1E293B] bg-white dark:bg-[#0F172A] sticky top-0 z-20 shrink-0">
      ...
      <main className="flex-1 p-4 sm:p-8 max-w-7xl w-full mx-auto flex flex-col relative">
        <div className="flex items-center gap-4 mb-6 sm:mb-8 shrink-0">
          <Link to="/dashboard" className="p-2 bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-[#1E293B] rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm">
            <ArrowLeft size={20} />
          </Link>
          <div className="flex gap-6 border-b border-transparent">
             <button onClick={() => setActiveTab('roster')} className={`text-xl font-bold tracking-tight pb-2 border-b-2 transition-colors ${activeTab === 'roster' ? 'border-blue-600 text-slate-900 dark:text-white' : 'border-transparent text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}>Roster</button>
             <button onClick={() => setActiveTab('memberships')} className={`text-xl font-bold tracking-tight pb-2 border-b-2 transition-colors ${activeTab === 'memberships' ? 'border-blue-600 text-slate-900 dark:text-white' : 'border-transparent text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}>Memberships</button>
          </div>
        </div>
        ...
        {activeTab === 'roster' && (
          <div className="flex flex-col flex-1 animate-in fade-in">
            <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6 shrink-0">
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:max-w-xl">
                <div className="relative w-full">
                  ...
                  <input type="text" placeholder={t('search_members')} value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-[#1E293B] rounded-lg shadow-sm outline-none focus:ring-2 focus:ring-blue-500 text-sm"/>
                </div>
                <select value={filterLevel} onChange={(e) => { setFilterLevel(e.target.value); setCurrentPage(1); }} className="w-full sm:w-48 px-3 py-2.5 bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-[#1E293B] rounded-lg shadow-sm outline-none focus:ring-2 focus:ring-blue-500 text-sm appearance-none font-medium">
            ...
            {/* Data Container */}
            <div className="bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-[#1E293B] rounded-xl shadow-sm overflow-hidden flex flex-col flex-1 min-h-[400px]">
              ...
              {loading ? (
                ...
                <>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-slate-50 dark:bg-[#0B1120] border-b border-slate-200 dark:border-[#1E293B] sticky top-0 z-10">
                        <tr className="text-xs uppercase tracking-wider text-slate-500 font-semibold">
                          <th className="p-4 cursor-pointer hover:bg-slate-100 dark:hover:bg-[#1E293B]/50 transition-colors" onClick={() => requestSort('name')}>
                          ...
                          <th className="p-4 cursor-pointer hover:bg-slate-100 dark:hover:bg-[#1E293B]/50 transition-colors" onClick={() => requestSort('skillLevel')}>
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-[#1E293B]">
                        {paginatedMembers.map((member) => (
                          <tr key={member.id} className="hover:bg-slate-50 dark:hover:bg-[#1E293B]/30 transition-colors">
                  ...
                  <div className="sm:hidden flex flex-col flex-1 divide-y divide-slate-100 dark:divide-[#1E293B] overflow-y-auto">
                    {paginatedMembers.map((member) => (
                      <div key={member.id} className="p-4 flex flex-col gap-3 hover:bg-slate-50 dark:hover:bg-[#1E293B]/20 transition-colors">
        ...
        {activeTab === 'memberships' && (
          <div className="flex flex-col lg:flex-row gap-6 flex-1 items-start animate-in fade-in">
            ...
            <div className="w-full lg:w-80 flex flex-col gap-4">
              <button onClick={() => setIsPeriodModalOpen(true)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl p-4 flex items-center justify-center gap-2 shadow-sm transition-colors">
                <Plus size={18} /> Create Membership Period
              </button>
              <div className="flex flex-col gap-3">
                {periods.length === 0 ? <p className="text-slate-500 text-sm text-center py-4">No periods found.</p> : 
                  periods.map(period => (
                    <div key={period.id} onClick={() => fetchPeriodPayments(period)} className={`p-4 rounded-xl border cursor-pointer transition-all flex justify-between items-center ${selectedPeriod?.id === period.id ? 'bg-white dark:bg-[#0F172A] border-blue-500 ring-1 ring-blue-500 shadow-sm' : 'bg-transparent border-slate-200 dark:border-[#1E293B] hover:bg-slate-50 dark:hover:bg-[#1E293B]'}`}>
                      <div className="flex flex-col">
                        <span className={`font-bold ${selectedPeriod?.id === period.id ? 'text-blue-600 dark:text-blue-400' : 'text-slate-900 dark:text-white'}`}>{period.name}</span>
                        <span className="text-xs text-slate-500 flex items-center gap-1 mt-1.5"><Calendar size={12}/> {new Date(period.startDate).toLocaleDateString()} - {new Date(period.endDate).toLocaleDateString()}</span>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); handleDeletePeriod(period.id); }} className="p-2 text-slate-400 hover:text-rose-500 transition-colors rounded-lg hover:bg-rose-50 dark:hover:bg-rose-900/20"><Trash2 size={16}/></button>
                    </div>
                  ))
                }
              </div>
            ...
            <div className="flex-1 w-full bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-[#1E293B] rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[400px]">
              {!selectedPeriod ? (
                <div className="flex-1 flex items-center justify-center text-slate-500">Select a membership period to view payments.</div>
              ) : (
                <>
                  <div className="p-6 border-b border-slate-200 dark:border-[#1E293B] bg-slate-50 dark:bg-[#0B1120] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <h2 className="text-xl font-bold dark:text-white">{selectedPeriod.name} Payments</h2>
                      <p className="text-sm text-slate-500 mt-1">{periodPayments.filter(p => p.status === 'paid').length} of {periodPayments.length} Paid</p>
                    </div>
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                      <div className="relative w-full sm:w-64">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                        <input type="text" placeholder="Search members..." value={membershipSearch} onChange={(e) => setMembershipSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-[#1E293B] rounded-lg shadow-sm outline-none focus:ring-2 focus:ring-blue-500 text-sm"/>
                      </div>
                    </div>
                  </div>
                  <div className="p-4 border-b border-slate-200 dark:border-[#1E293B] bg-white dark:bg-[#0F172A]">
                     <button onClick={openAddPeriodMemberModal} className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-[#1E293B] dark:hover:bg-[#334155] text-slate-800 dark:text-slate-200 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm w-full sm:w-auto justify-center">
                  ...
                  <div className="flex-1 overflow-y-auto">
                    <table className="w-full text-left">
                      <tbody className="divide-y divide-slate-100 dark:divide-[#1E293B]">
                        {filteredPeriodPayments.length === 0 && <tr><td className="p-8 text-center text-slate-500">No members in this period.</td></tr>}
                        {filteredPeriodPayments.map(p => (
                          <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-[#1E293B]/30 group transition-colors">
                            <td className="p-4 font-bold text-slate-900 dark:text-white">{p.memberName}</td>
                            <td className="p-4 text-right">
                              <div className="flex items-center justify-end gap-4">
                                <button onClick={() => handleTogglePaymentStatus(p.id, p.status)} className={`px-4 py-1.5 rounded font-bold text-xs uppercase tracking-widest transition-colors ${p.status === 'paid' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-[#1E293B] dark:text-slate-400'}`}>
                                  {p.status}
                                </button>
                                <button onClick={() => handleRemoveMemberFromPeriod(p.id)} className="text-slate-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><X size={18}/></button>
                              </div>
                            </td>
                          </tr>
                        ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90dvh]">
            ...
            <div className="p-5 border-t border-slate-100 dark:border-[#1E293B] bg-slate-50 dark:bg-[#0B1120] flex justify-end gap-3 shrink-0">
      ...
      {isPeriodModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#0F172A] w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-[#1E293B]">
            <div className="flex justify-between items-center p-5 border-b border-slate-100 dark:border-[#1E293B] bg-slate-50 dark:bg-[#0B1120]">
              <h3 className="font-bold text-lg">Create Period</h3>
              <button onClick={() => setIsPeriodModalOpen(false)} className="p-1.5 text-slate-400 hover:bg-slate-200 dark:hover:bg-[#1E293B] rounded-full transition-colors"><X size={18} /></button>
            </div>
            <div className="p-6">
            ...
            <div className="p-5 border-t border-slate-100 dark:border-[#1E293B] bg-slate-50 dark:bg-[#0B1120] flex justify-end gap-3">
              <button type="button" onClick={() => setIsPeriodModalOpen(false)} className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-[#1E293B] rounded-lg transition-colors">Cancel</button>
              <button type="submit" form="period-form" className="px-5 py-2 text-sm font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-sm transition-colors">Create</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-[#0F172A] w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[85dvh] overflow-hidden border border-slate-200 dark:border-[#1E293B]">
            <div className="flex justify-between items-center p-5 border-b border-slate-200 dark:border-[#1E293B] bg-slate-50 dark:bg-[#0B1120]">
              ...
              <button disabled={isProcessing} onClick={() => setAddPeriodMemberModalOpen(false)} className="p-1.5 text-slate-400 hover:bg-slate-200 dark:hover:bg-[#1E293B] rounded-full transition-colors disabled:opacity-50"><X size={18}/></button>
            </div>
            
            <div className="p-4 border-b border-slate-200 dark:border-[#1E293B]">
              <div className="relative w-full">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                <input disabled={isProcessing} type="text" placeholder={t('search_players')} value={periodMemberSearch} onChange={(e) => setPeriodMemberSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-[#0B1120] border border-slate-200 dark:border-[#1E293B] rounded-lg shadow-sm outline-none focus:ring-2 focus:ring-blue-500 text-sm disabled:opacity-50" autoFocus />
              </div>
            </div>
            ...
            <div className="p-2 overflow-y-auto flex-1 bg-white dark:bg-[#0F172A]">
              {(() => {
                ...
                if (available.length === 0) return <div className="p-8 text-center text-slate-500">{t('no_players')}</div>;
                return available.map(member => (
                  <div key={member.id} className={`flex items-center p-3 hover:bg-slate-50 dark:hover:bg-[#1E293B]/50 rounded-xl cursor-pointer transition-colors ${isProcessing ? 'pointer-events-none opacity-50' : ''}`} onClick={() => toggleSelectPeriodMember(member.id)}>
                    <div className="flex items-center gap-4 w-full">
                      <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedPeriodMembers.includes(member.id) ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-300 dark:border-[#334155]'}`}>
            ...
            <div className="p-4 border-t border-slate-200 dark:border-[#1E293B] bg-slate-50 dark:bg-[#0B1120]">
              <button onClick={handleAddSelectedPeriodMembers} disabled={selectedPeriodMembers.length === 0 || isProcessing} className="w-full py-3 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
