import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';

const resources = {
  en: {
    translation: {
      // General & Auth
      "login": "Login",
      "register": "Register",
      "email": "Email Address",
      "username": "Username",
      "password": "Password",
      "have_account": "Already have an account?",
      "no_account": "Don't have an account?",
      "welcome": "Welcome",
      "logout": "Logout",
      "dashboard_init": "Dashboard functionality initialized.",
      "loading": "Loading...",
      "continue": "Continue",
      "edit": "Edit",
      "cancel": "Cancel",
      "save": "Save",
      "create": "Create",
      "confirm": "Confirm",
      "abort": "Abort",
      "other": "Other",
      "optional": "(Optional)",
      "processing": "Processing...",
      "switch_language": "Switch language",
      "use_light_mode": "Use light mode",
      "use_dark_mode": "Use dark mode",
      "auth_kicker": "Community match management",
      "auth_heading": "Make every session feel effortless.",
      "auth_description": "Keep your roster organized, build fair matchups, and turn weekly play into a community ritual.",
      "auth_benefit_members": "One clear roster",
      "auth_benefit_scores": "Smarter score history",
      "auth_benefit_ready": "Ready when you are",
      "auth_footer_note": "Built for communities that play together",
      "auth_form_label": "Welcome back",
      "forgot_password": "Forgot password?",
      "login_identifier_hint": "Use your email or username",
      "login_password_hint": "Enter your password",
      "register_password_hint": "Create a password",
      "show_password": "Show password",
      "hide_password": "Hide password",
      "forgot_password_title": "Reset your password",
      "forgot_password_description": "Enter your email and we’ll send you a link to reset your password.",
      "send_reset_link": "Send reset link",
      "back_to_login": "Back to login",
      "reset_password_title": "Create a new password",
      "reset_password_description": "Choose a new password for your AturMabar account.",
      "new_password_label": "New password",
      "reset_password": "Reset password",
      "invalid_reset_token": "This reset link is invalid or has expired.",
      "verify_email_title": "Verify your email",
      "verify_email_change_title": "Verify email change",
      "verification_pending": "Verifying your email address...",
      "verification_success_redirect": "Redirecting you to login...",
      "verification_failed": "We could not verify this link.",

      // Subscription & Access
      "sub_inactive": "Subscription Inactive",
      "sub_desc": "Your community's access has expired or is inactive. Please contact the administrator to renew your subscription.",
      "return_login": "Return to Login",

      // Settings & Profile
      "settings": "Settings",
      "profile": "Profile",
      "account": "Account",
      "general": "General",
      "appearance": "Appearance",
      "language": "Language",
      "light_mode": "Light",
      "dark_mode": "Dark",
      "community_name": "Community Name",
      "change_logo": "Change Logo",
      "save_changes": "Save Changes",
      "current_email": "Current Email",
      "new_email": "New Email",
      "send_verification": "Send Verification",
      "old_password": "Old Password",
      "new_password": "New Password",
      "confirm_password": "Confirm Password",
      "update_password": "Update Password",
      "public_id_desc": "Manage your community's public identity.",
      "upload_image": "Upload Image",
      "account_desc": "Update your email address and secure your account.",
      "username_cant_change": "Username (Cannot be changed)",
      "email_unbind_warning": "Changing this will unbind your previous email.",
      "leave_blank_pass": "Leave blank to keep current",
      "app_pref_desc": "Configure your application preferences.",
      "updating": "Updating...",
      "saving": "Saving...",
      
      // Alerts & Popups
      "login_success": "Login successful",
      "reg_success": "Registration successful. Please verify your email.",
      "pass_unmatch": "Passwords do not match.",
      "fill_details": "Please fill in all details.",
      "delete_confirm": "Permanently delete this account and community?",
      "delete_failed": "Delete failed.",
      "op_failed": "Operation failed.",
      "invalid_creds": "Invalid credentials.",
      "verify_email_success": "Email successfully updated.",
      "score_updated": "Score updated successfully.",
      "match_cancelled": "Match cancelled successfully.",
      "server_error": "Server error",
      
      // Admin Dashboard
      "sys_admin": "System Admin",
      "comm_accounts": "Community Accounts",
      "manage_desc": "Manage users, communities, and system access.",
      "new_account": "New Account",
      "search_ph": "Search by name, username, or email...",
      "no_accounts": "No matching accounts found.",
      "status": "Status",
      "active": "Active",
      "inactive": "Inactive",
      "lifetime": "Lifetime",
      "edit_account": "Edit Account",
      "create_account": "Create Account",
      "profile_data": "Profile Data",
      "security": "Security",
      "override_pass": "Override Password",
      "leave_blank_unchanged": "Leave blank to keep unchanged",
      "comm_settings": "Community Settings",
      "sys_access": "System Access",
      "sub_action": "Subscription Action",
      "keep_status": "Keep Current Status",
      "select_initial": "Select Initial Status...",
      "revoke": "Revoke Access (Inactive)",
      "custom_date": "Expiration Date",
      
      // Verification Page
      "verify_title": "Verify Email Change",
      "verifying": "Verifying your new email address...",
      "redirect_login": "Redirecting to login...",
      "missing_token": "Missing token in URL parameters.",

      // Registration Details
      "fill_account_details": "Please fill in all account details.",
      "comm_name_req": "Community name is required.",
      "ph_email": "name@example.com",
      "ph_username": "Choose a username",
      "ph_comm_name": "e.g., Kokobadminton Community",
      "social_links": "Social Links",
      "add_link": "Add Link",
      "no_social_links": "No social links added yet.",
      "complete_reg": "Complete Registration",
      "select_logo": "Select Community Logo",
      "or_upload": "Or upload custom",

      // Dashboard
      "dashboard": "Dashboard",
      "overview": "Overview",
      "quick_actions": "Quick Actions",
      "manage_members": "Manage Members",
      "manage_schedule": "Schedule & Sessions",
      "manage_tournaments": "Tournaments",
      "community_overview": "Community Overview",
      "subscription_status": "Subscription Status",
      "view_all": "View All",
      "recent_activity": "Recent Activity",
      "no_activity": "No recent activity found.",

      // Roster & Members
      "members": "Members",
      "roster_management": "Roster Management",
      "roster_desc": "Manage your community roster and player levels.",
      "add_member": "Add Member",
      "edit_member": "Edit Member",
      "search_members": "Search members by name or phone...",
      "no_members": "No members found in this community.",
      "name": "Full Name",
      "phone": "Phone Number",
      "skill_level": "Skill Level",
      "beginner": "Beginner",
      "intermediate": "Intermediate",
      "advanced": "Advanced",
      "del_member_confirm": "Are you sure you want to remove this member?",
      "ph_name": "e.g., John Doe",
      "ph_phone": "e.g., +628123456789",
      "add_player": "Add Player",
      "gender": "Gender",
      "male": "Male",
      "female": "Female",
      "avoid_partner": "Avoid as Partner",
      "avoid_opponent": "Avoid as Opponent",
      "pairing_restrictions": "Pairing Restrictions",
      "search_restrict": "Search players to restrict...",
      "save_player": "Save Player",
      "edit_player": "Edit Player",
      "all_levels": "All Levels",
      "no_players": "No players found.",
      "page_of": "Page {{current}} of {{total}}",

      // Sessions
      "sessions": "Sessions",
      "session_mgmt": "Session & Schedule",
      "create_session": "Create Session",
      "session_name": "Session Name",
      "session_name_placeholder": "e.g. Sunday Morning Social",
      "date_time": "Date & Time",
      "court_count": "Number of Courts",
      "session_setup": "Session setup",
      "session_setup_progress": "Session setup progress",
      "all_games": "All games",
      "custom_amount": "Custom amount",
      "match_limit": "Match limit",
      "available_players": "Available players",
      "waiting_list": "Waiting list",
      "absent": "Absent",
      "member_unpaid": "Member (unpaid)",
      "played": "Played",
      "attendance_added": "Attendance added",
      "billing_reset": "Billing reset",
      "court_added": "Court added",
      "court_deleted": "Court deleted",
      "court_swapped": "Court swapped",
      "court_updated": "Court updated",
      "import_members": "Import members",
      "match_finished": "Match finished",
      "match_queued": "Match queued",
      "match_started": "Match started",
      "match_updated": "Match updated",
      "reset_billing": "Reset billing",
      "reset_billing_confirm": "Reset all billing records?",
      "restart_session": "Restart session",
      "set_resting": "Set resting",
      "status_updated": "Status updated",
      "walk_in_added": "Walk-in added",
      "empty": "Empty",
      "session_setup_basics": "Basics",
      "session_setup_basics_desc": "Give the session a clear name and choose when it will take place.",
      "session_setup_courts": "Courts",
      "session_setup_courts_desc": "Set the number of courts available for this session.",
      "session_setup_courts_tip": "Start with the courts you know are available",
      "session_setup_courts_tip_desc": "You can adjust courts later from the session details page.",
      "session_setup_roster": "Roster",
      "session_setup_roster_desc": "Choose members who are expected to play. You can add or remove players later.",
      "session_setup_no_members": "No members are available yet.",
      "session_setup_selected_members": "{{count}} members selected",
      "session_setup_rules": "Rules",
      "session_setup_rules_desc": "Choose scoring and pairing preferences for this session.",
      "session_setup_fees": "Fees",
      "session_setup_fees_desc": "Set optional default fees. You can update billing later.",
      "session_setup_summary": "Review session",
      "match_limit_placeholder": "Number of games",
      "next": "Next",
      "back": "Back",
      "start_tour": "Start Tour",
      "tour_title": "Interactive App Tour",
      "tour_welcome": "Welcome to AturMabar! We will guide you through the main workflow for managing your community.",
      "tour_walkthrough_title": "Start App Walkthrough",
      "tour_walkthrough_desc": "Take a guided tour to learn how to manage your community.",
      "tour_cancel": "Cancel Tour",
      "tour_end": "End Tour",
      "organizer_view": "Organizer view",
      "fairness_insights": "Fairness insights",
      "fairness_insights_desc": "Review the rotation signals behind the current queue and pairings.",
      "fairness_insights_empty": "Insights will appear after players have played a match.",
      "fairness_insights_note": "These are neutral signals to support organizer decisions, not fixed rankings or judgments.",
      "court_turns": "Court turns",
      "rest_time": "Rest time",
      "partner_repetition": "Partner repeats",
      "opponent_repetition": "Opponent repeats",
      "queue_position": "Queue #{{position}}",
      "not_played_yet": "Not played yet",
      "minutes_value": "{{minutes}} min",
      "close": "Close",
      "scoring_system": "Default Scoring System",
      "pairing_rule": "Matchmaking Strictness",
      "very_strict": "Very Strict (Same Grade Only)",
      "strict": "Strict (+/- 1 Grade)",
      "moderate": "Moderate (+/- 2 Grades)",
      "randomize": "Randomize",
      "bwf_21": "BWF 21 Points x 3 Sets",
      "bwf_15": "BWF 15 Points x 3 Sets",
      "pts_42": "42 Points x 1 Set",
      "pts_30": "30 Points x 1 Set",
      "custom": "Custom Sets & Points",
      "custom_sets": "Sets",
      "custom_points": "Points per Set",
      "status_scheduled": "Scheduled",
      "status_active": "Active",
      "status_ended": "Ended",
      "search_sessions": "Search session name...",
      "filter_date": "Filter by date",
      "no_sessions": "No sessions found.",
      "start_session": "Start Session",
      "manage_session": "Manage Session",
      "delete_session": "Delete Session",
      "end_session": "End Session",
      "status_finished": "FINISHED",
      "session_not_started": "Start the session to enable matchmaking.",
      "session_schedule": "Session & Schedule",
      "save_settings": "Save Settings",
      "session_settings": "Session Settings",
      "danger_zone": "Danger Zone",
      "delete_session_warning": "Permanently delete this session and all its data.",

      // Matchmaking & Courts
      "attendance": "Attendance",
      "courts": "Courts",
      "matches": "Matches",
      "history": "History",
      "playtime": "Playtime",
      "mark_active": "Mark Active",
      "mark_resting": "Mark Resting",
      "cancel_attendance": "Cancel",
      "add_attendee": "Add Attendee",
      "add_walk_in": "Add Walk-In",
      "arrived_at": "Arrived At",
      "resting": "Resting",
      "active_court": "Active",
      "inactive_court": "Inactive",
      "edit_court": "Edit Court",
      "search_players": "Search players...",
      "add_selected": "Add Selected ({{count}})",
      "add_court": "Add Court",
      "delete_court": "Delete Court",
      "auto_fill": "Auto-Fill Courts",
      "queue_match": "Queue a Match",
      "manual_match": "Manual Match",
      "ready": "READY",
      "vs": "VS",
      "start": "Start",
      "finish_free_court": "Finish & Free Court",
      "end_without_score": "End without score",
      "search_players_to_select": "Search players to select...",
      "remove_player": "- Remove Player -",
      "swap_with": "Swap with",
      "cancel_match": "Cancel Match",
      "save_score": "Save Score",
      "confirm_score_title": "Confirm Score",
      "confirm_score_desc": "Are you sure you want to save this score?",
      "confirm_cancel_title": "Cancel Match",
      "confirm_cancel_desc": "Are you sure you want to cancel this match? It will not be recorded in playtime history.",
      "matches_played": "Matches Played",
      "no_history": "No matches have been completed yet.",
      "edit_history": "Edit History",
      "court_time": "Court & Time",
      "team_a": "Team A",
      "team_b": "Team B",
      "score": "Score",
      "export_pdf": "Export PDF",
      "player_details": "Player Details",
      "win_rate": "Win Rate",
      "won": "Won",
      "lost": "Lost",
      "draw": "Draw",
      "partner": "Partner",
      "opponents": "Opponents",

      // Leaderboard
      "leaderboard": "Leaderboard",
      "by_month": "By Month",
      "by_session": "By Session",
      "select_month": "Select Month",
      "select_session": "Select Session",
      "rank": "Rank",
      "player": "Player",
      "w_l": "W-L",
      "net_sets": "Net Sets",
      "net_pts": "Net Pts",
      "total_pts": "Total Pts",

      // 404 & Errors
      "not_found_title": "Page Not Found",
      "not_found_desc": "The page you are looking for doesn't exist or has been moved.",
      "return_home": "Return Home",

      // Session Billing Page
      "billing": "Billing",
      "financial_summary": "Financial Summary",
      "total_income": "Total Income",
      "total_expense": "Total Expense",
      "net_balance": "Net Balance",
      "player_payments": "Player Payments",
      "expenses": "Expenses",
      "default_fee": "Default Fee",
      "set_fee": "Set Fee",
      "amount": "Amount",
      "paid": "Paid",
      "unpaid": "Unpaid",
      "free": "Free",
      "add_expense": "Add Expense",
      "description": "Description",
      "expense_added": "Expense added",
      "expense_deleted": "Expense deleted",
      "fee_updated": "Default fee updated",
      "member": "Member",
      "member_fee": "Member Fee",
      "walk_in_fee": "Walk-in Fee",
    }
  },
  id: {
    translation: {
      // General & Auth
      "login": "Masuk",
      "register": "Daftar",
      "email": "Alamat Email",
      "username": "Nama Pengguna",
      "password": "Kata Sandi",
      "have_account": "Sudah punya akun?",
      "no_account": "Belum punya akun?",
      "welcome": "Selamat Datang",
      "logout": "Keluar",
      "dashboard_init": "Fungsi dasbor telah diinisialisasi.",
      "loading": "Memuat...",
      "continue": "Lanjutkan",
      "edit": "Edit",
      "cancel": "Batal",
      "save": "Simpan",
      "create": "Buat",
      "confirm": "Konfirmasi",
      "abort": "Batal",
      "other": "Lainnya",
      "optional": "(Opsional)",
      "processing": "Memproses...",
      "switch_language": "Ganti bahasa",
      "use_light_mode": "Gunakan mode terang",
      "use_dark_mode": "Gunakan mode gelap",
      "auth_kicker": "Manajemen mabar komunitas",
      "auth_heading": "Buat setiap sesi terasa lebih mudah.",
      "auth_description": "Rapikan roster, buat pasangan yang adil, dan ubah mabar mingguan menjadi rutinitas komunitas.",
      "auth_benefit_members": "Roster yang rapi",
      "auth_benefit_scores": "Riwayat skor cerdas",
      "auth_benefit_ready": "Siap kapan saja",
      "auth_footer_note": "Dibuat untuk komunitas yang bermain bersama",
      "auth_form_label": "Selamat datang kembali",
      "forgot_password": "Lupa kata sandi?",
      "login_identifier_hint": "Gunakan email atau nama pengguna",
      "login_password_hint": "Masukkan kata sandi Anda",
      "register_password_hint": "Buat kata sandi",
      "show_password": "Tampilkan kata sandi",
      "hide_password": "Sembunyikan kata sandi",
      "forgot_password_title": "Atur ulang kata sandi",
      "forgot_password_description": "Masukkan email Anda dan kami akan mengirimkan tautan untuk mengatur ulang kata sandi.",
      "send_reset_link": "Kirim tautan reset",
      "back_to_login": "Kembali ke masuk",
      "reset_password_title": "Buat kata sandi baru",
      "reset_password_description": "Pilih kata sandi baru untuk akun AturMabar Anda.",
      "new_password_label": "Kata sandi baru",
      "reset_password": "Atur ulang kata sandi",
      "invalid_reset_token": "Tautan reset ini tidak valid atau sudah kedaluwarsa.",
      "verify_email_title": "Verifikasi email Anda",
      "verify_email_change_title": "Verifikasi perubahan email",
      "verification_pending": "Memverifikasi alamat email Anda...",
      "verification_success_redirect": "Anda akan diarahkan ke halaman masuk...",
      "verification_failed": "Kami tidak dapat memverifikasi tautan ini.",

      // Subscription & Access
      "sub_inactive": "Langganan Tidak Aktif",
      "sub_desc": "Akses komunitas Anda telah berakhir atau tidak aktif. Silakan hubungi administrator untuk memperbarui langganan Anda.",
      "return_login": "Kembali ke Masuk",

      // Settings & Profile
      "settings": "Pengaturan",
      "profile": "Profil",
      "account": "Akun",
      "general": "Umum",
      "appearance": "Tampilan",
      "language": "Bahasa",
      "light_mode": "Terang",
      "dark_mode": "Gelap",
      "community_name": "Nama Komunitas",
      "change_logo": "Ubah Logo",
      "save_changes": "Simpan Perubahan",
      "current_email": "Email Saat Ini",
      "new_email": "Email Baru",
      "send_verification": "Kirim Verifikasi",
      "old_password": "Kata Sandi Lama",
      "new_password": "Kata Sandi Baru",
      "confirm_password": "Konfirmasi Sandi",
      "update_password": "Perbarui Sandi",
      "public_id_desc": "Kelola identitas publik komunitas Anda.",
      "upload_image": "Unggah Gambar",
      "account_desc": "Perbarui alamat email Anda dan amankan akun Anda.",
      "username_cant_change": "Nama Pengguna (Tidak dapat diubah)",
      "email_unbind_warning": "Mengubah ini akan melepaskan email sebelumnya.",
      "leave_blank_pass": "Biarkan kosong untuk mempertahankan yang sekarang",
      "app_pref_desc": "Konfigurasikan preferensi aplikasi Anda.",
      "updating": "Memperbarui...",
      "saving": "Menyimpan...",
      
      // Alerts & Popups
      "login_success": "Berhasil masuk.",
      "reg_success": "Pendaftaran berhasil. Silakan periksa email Anda.",
      "pass_unmatch": "Kata sandi tidak cocok.",
      "fill_details": "Harap lengkapi semua data.",
      "delete_confirm": "Hapus permanen akun dan komunitas ini?",
      "delete_failed": "Gagal menghapus.",
      "op_failed": "Operasi gagal.",
      "invalid_creds": "Kredensial tidak valid.",
      "verify_email_success": "Email berhasil diperbarui.",
      "score_updated": "Skor berhasil diperbarui.",
      "match_cancelled": "Pertandingan berhasil dibatalkan.",
      "server_error": "Kesalahan server",
      
      // Admin Dashboard
      "sys_admin": "Admin Sistem",
      "comm_accounts": "Akun Komunitas",
      "manage_desc": "Kelola pengguna, komunitas, dan akses sistem.",
      "new_account": "Akun Baru",
      "search_ph": "Cari nama, pengguna, atau email...",
      "no_accounts": "Tidak ada akun yang cocok ditemukan.",
      "status": "Status",
      "active": "Aktif",
      "inactive": "Tidak Aktif",
      "lifetime": "Seumur Hidup",
      "edit_account": "Edit Akun",
      "create_account": "Buat Akun",
      "profile_data": "Data Profil",
      "security": "Keamanan",
      "override_pass": "Timpa Kata Sandi",
      "leave_blank_unchanged": "Biarkan kosong jika tidak diubah",
      "comm_settings": "Pengaturan Komunitas",
      "sys_access": "Akses Sistem",
      "sub_action": "Tindakan Langganan",
      "keep_status": "Pertahankan Status Saat Ini",
      "select_initial": "Pilih Status Awal...",
      "revoke": "Cabut Akses (Tidak Aktif)",
      "custom_date": "Tanggal Berakhir",
      
      // Verification Page
      "verify_title": "Verifikasi Perubahan Email",
      "verifying": "Memverifikasi alamat email baru Anda...",
      "redirect_login": "Mengarahkan ke halaman masuk...",
      "missing_token": "Token tidak ditemukan pada URL.",

      // Registration Details
      "fill_account_details": "Harap lengkapi semua detail akun.",
      "comm_name_req": "Nama komunitas wajib diisi.",
      "ph_email": "nama@contoh.com",
      "ph_username": "Pilih nama pengguna",
      "ph_comm_name": "misal, Komunitas Kokobadminton",
      "social_links": "Tautan Sosial",
      "add_link": "Tambah Tautan",
      "no_social_links": "Belum ada tautan sosial yang ditambahkan.",
      "complete_reg": "Selesaikan Pendaftaran",
      "select_logo": "Pilih Logo Komunitas",
      "or_upload": "Atau unggah khusus",

      // Dashboard
      "dashboard": "Dasbor",
      "overview": "Ringkasan",
      "quick_actions": "Aksi Cepat",
      "manage_members": "Kelola Anggota",
      "manage_schedule": "Jadwal & Sesi",
      "manage_tournaments": "Turnamen",
      "community_overview": "Ringkasan Komunitas",
      "subscription_status": "Status Langganan",
      "view_all": "Lihat Semua",
      "recent_activity": "Aktivitas Terbaru",
      "no_activity": "Tidak ada aktivitas terbaru.",

      // Roster & Members
      "members": "Anggota",
      "roster_management": "Manajemen Pemain",
      "roster_desc": "Kelola daftar anggota komunitas dan tingkat kemampuan pemain.",
      "add_member": "Tambah Anggota",
      "edit_member": "Edit Anggota",
      "search_members": "Cari anggota berdasarkan nama atau telepon...",
      "no_members": "Tidak ada anggota yang ditemukan di komunitas ini.",
      "name": "Nama Lengkap",
      "phone": "Nomor Telepon",
      "skill_level": "Tingkat Kemampuan",
      "beginner": "Pemula",
      "intermediate": "Menengah",
      "advanced": "Mahir",
      "del_member_confirm": "Apakah Anda yakin ingin menghapus anggota ini?",
      "ph_name": "misal, Budi Santoso",
      "ph_phone": "misal, +628123456789",
      "add_player": "Tambah Pemain",
      "gender": "Jenis Kelamin",
      "male": "Laki-laki",
      "female": "Perempuan",
      "avoid_partner": "Hindari sebagai Pasangan",
      "avoid_opponent": "Hindari sebagai Lawan",
      "pairing_restrictions": "Batasan Pasangan",
      "search_restrict": "Cari pemain untuk dibatasi...",
      "save_player": "Simpan Pemain",
      "edit_player": "Edit Pemain",
      "all_levels": "Semua Level",
      "no_players": "Tidak ada pemain ditemukan.",
      "page_of": "Halaman {{current}} dari {{total}}",

      // Sessions
      "sessions": "Sesi",
      "session_mgmt": "Sesi & Jadwal",
      "create_session": "Buat Sesi",
      "session_name": "Nama Sesi",
      "session_name_placeholder": "contoh: Mabar Minggu Pagi",
      "date_time": "Tanggal & Waktu",
      "court_count": "Jumlah Lapangan",
      "session_setup": "Pengaturan sesi",
      "session_setup_progress": "Kemajuan pengaturan sesi",
      "all_games": "Semua permainan",
      "custom_amount": "Jumlah khusus",
      "match_limit": "Batas permainan",
      "available_players": "Pemain tersedia",
      "waiting_list": "Daftar tunggu",
      "absent": "Tidak hadir",
      "member_unpaid": "Member (belum dibayar)",
      "played": "Dimainkan",
      "attendance_added": "Kehadiran ditambahkan",
      "billing_reset": "Tagihan direset",
      "court_added": "Lapangan ditambahkan",
      "court_deleted": "Lapangan dihapus",
      "court_swapped": "Lapangan ditukar",
      "court_updated": "Lapangan diperbarui",
      "import_members": "Impor anggota",
      "match_finished": "Pertandingan selesai",
      "match_queued": "Pertandingan masuk antrean",
      "match_started": "Pertandingan dimulai",
      "match_updated": "Pertandingan diperbarui",
      "reset_billing": "Reset tagihan",
      "reset_billing_confirm": "Reset semua catatan tagihan?",
      "restart_session": "Mulai ulang sesi",
      "set_resting": "Atur istirahat",
      "status_updated": "Status diperbarui",
      "walk_in_added": "Pemain walk-in ditambahkan",
      "empty": "Kosong",
      "session_setup_basics": "Dasar",
      "session_setup_basics_desc": "Beri nama sesi yang jelas dan pilih waktu pelaksanaannya.",
      "session_setup_courts": "Lapangan",
      "session_setup_courts_desc": "Tentukan jumlah lapangan yang tersedia untuk sesi ini.",
      "session_setup_courts_tip": "Mulai dari lapangan yang sudah tersedia",
      "session_setup_courts_tip_desc": "Jumlah lapangan masih dapat disesuaikan dari halaman detail sesi.",
      "session_setup_roster": "Roster",
      "session_setup_roster_desc": "Pilih anggota yang diperkirakan bermain. Pemain masih dapat ditambah atau dihapus nanti.",
      "session_setup_no_members": "Belum ada anggota yang tersedia.",
      "session_setup_selected_members": "{{count}} anggota dipilih",
      "session_setup_rules": "Aturan",
      "session_setup_rules_desc": "Pilih preferensi poin dan pasangan untuk sesi ini.",
      "session_setup_fees": "Tarif",
      "session_setup_fees_desc": "Atur tarif default jika diperlukan. Penagihan masih dapat diperbarui nanti.",
      "session_setup_summary": "Tinjau sesi",
      "match_limit_placeholder": "Jumlah permainan",
      "next": "Lanjut",
      "back": "Kembali",
      "start_tour": "Mulai Tur",
      "tour_title": "Tur Interaktif Aplikasi",
      "tour_welcome": "Selamat datang di AturMabar! Kami akan memandu alur utama untuk mengelola komunitas Anda.",
      "tour_walkthrough_title": "Mulai Panduan Aplikasi",
      "tour_walkthrough_desc": "Ikuti tur singkat untuk mempelajari cara mengelola komunitas Anda.",
      "tour_cancel": "Batalkan Tur",
      "tour_end": "Akhiri Tur",
      "organizer_view": "Tampilan organizer",
      "fairness_insights": "Wawasan rotasi",
      "fairness_insights_desc": "Tinjau sinyal rotasi di balik antrean dan pasangan saat ini.",
      "fairness_insights_empty": "Wawasan akan muncul setelah pemain menyelesaikan pertandingan.",
      "fairness_insights_note": "Ini adalah sinyal netral untuk membantu keputusan organizer, bukan peringkat atau penilaian tetap.",
      "court_turns": "Giliran lapangan",
      "rest_time": "Waktu istirahat",
      "partner_repetition": "Pengulangan partner",
      "opponent_repetition": "Pengulangan lawan",
      "queue_position": "Antrean #{{position}}",
      "not_played_yet": "Belum bermain",
      "minutes_value": "{{minutes}} mnt",
      "close": "Tutup",
      "scoring_system": "Sistem Poin Default",
      "pairing_rule": "Tingkat Ketat Pasangan",
      "very_strict": "Sangat Ketat (Hanya Grade Sama)",
      "strict": "Ketat (+/- 1 Grade)",
      "moderate": "Moderat (+/- 2 Grade)",
      "randomize": "Acak",
      "bwf_21": "BWF 21 Poin x 3 Set",
      "bwf_15": "BWF 15 Poin x 3 Set",
      "pts_42": "42 Poin x 1 Set",
      "pts_30": "30 Poin x 1 Set",
      "custom": "Set & Poin Khusus",
      "custom_sets": "Jumlah Set",
      "custom_points": "Poin per Set",
      "status_scheduled": "Dijadwalkan",
      "status_active": "Aktif",
      "status_ended": "Selesai",
      "search_sessions": "Cari nama sesi...",
      "filter_date": "Filter tanggal",
      "no_sessions": "Tidak ada sesi ditemukan.",
      "start_session": "Mulai Sesi",
      "manage_session": "Kelola Sesi",
      "delete_session": "Hapus Sesi",
      "end_session": "Akhiri Sesi",
      "status_finished": "SELESAI",
      "session_not_started": "Mulai sesi untuk mengaktifkan fitur pertandingan.",
      "session_schedule": "Sesi & Jadwal",
      "save_settings": "Simpan Pengaturan",
      "session_settings": "Pengaturan Sesi",
      "danger_zone": "Zona Berbahaya",
      "delete_session_warning": "Hapus permanen sesi ini beserta semua datanya.",

      // Matchmaking & Courts
      "attendance": "Kehadiran",
      "courts": "Lapangan",
      "matches": "Pertandingan",
      "history": "Riwayat",
      "playtime": "Waktu Main",
      "mark_active": "Tandai Aktif",
      "mark_resting": "Tandai Istirahat",
      "cancel_attendance": "Batal",
      "add_attendee": "Tambah Peserta",
      "add_walk_in": "Tambah Walk-In",
      "arrived_at": "Tiba Pukul",
      "resting": "Istirahat",
      "active_court": "Aktif",
      "inactive_court": "Tidak Aktif",
      "edit_court": "Edit Lapangan",
      "search_players": "Cari pemain...",
      "add_selected": "Tambah Pilihan ({{count}})",
      "add_court": "Tambah Lapangan",
      "delete_court": "Hapus Lapangan",
      "auto_fill": "Isi Otomatis",
      "queue_match": "Antrekan Pertandingan",
      "manual_match": "Pertandingan Manual",
      "ready": "SIAP",
      "vs": "VS",
      "start": "Mulai",
      "finish_free_court": "Selesai & Kosongkan",
      "end_without_score": "Akhiri tanpa skor",
      "search_players_to_select": "Cari pemain untuk dipilih...",
      "remove_player": "- Hapus Pemain -",
      "swap_with": "Tukar dengan",
      "cancel_match": "Batalkan Pertandingan",
      "save_score": "Simpan Skor",
      "confirm_score_title": "Konfirmasi Skor",
      "confirm_score_desc": "Apakah Anda yakin ingin menyimpan skor ini?",
      "confirm_cancel_title": "Batalkan Pertandingan",
      "confirm_cancel_desc": "Apakah Anda yakin ingin membatalkan pertandingan ini? Ini tidak akan dicatat dalam riwayat permainan.",
      "matches_played": "Main",
      "no_history": "Belum ada pertandingan yang selesai.",
      "edit_history": "Edit Riwayat",
      "court_time": "Lapangan & Waktu",
      "team_a": "Tim A",
      "team_b": "Tim B",
      "score": "Skor",
      "export_pdf": "Ekspor PDF",
      "player_details": "Detail Pemain",
      "win_rate": "Rasio Menang",
      "won": "Menang",
      "lost": "Kalah",
      "draw": "Seri",
      "partner": "Pasangan",
      "opponents": "Lawan",

      // Leaderboard
      "leaderboard": "Papan Peringkat",
      "by_month": "Per Bulan",
      "by_session": "Per Sesi",
      "select_month": "Pilih Bulan",
      "select_session": "Pilih Sesi",
      "rank": "Peringkat",
      "player": "Pemain",
      "w_l": "M-K",
      "net_sets": "Selisih Set",
      "net_pts": "Selisih Poin",
      "total_pts": "Total Poin",

      // 404 & Errors
      "not_found_title": "Halaman Tidak Ditemukan",
      "not_found_desc": "Halaman yang Anda cari tidak ada atau telah dipindahkan.",
      "return_home": "Kembali ke Beranda",

      // Session Billing Page
      "billing": "Penagihan",
      "financial_summary": "Ringkasan Keuangan",
      "total_income": "Total Pemasukan",
      "total_expense": "Total Pengeluaran",
      "net_balance": "Saldo Bersih",
      "player_payments": "Pembayaran Pemain",
      "expenses": "Pengeluaran",
      "default_fee": "Tarif Default",
      "set_fee": "Atur Tarif",
      "amount": "Jumlah",
      "paid": "Dibayar",
      "unpaid": "Belum Dibayar",
      "free": "Gratis",
      "add_expense": "Tambah Pengeluaran",
      "description": "Deskripsi",
      "expense_added": "Pengeluaran ditambahkan",
      "expense_deleted": "Pengeluaran dihapus",
      "fee_updated": "Tarif default diperbarui",
      "member": "Member",
      "member_fee": "Tarif Member",
      "walk_in_fee": "Tarif Walk-in",
    }
  }
};

const savedLanguage = localStorage.getItem('language') || 'en';

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: savedLanguage,
    fallbackLng: 'en',
    interpolation: {
      escapeValue: false
    }
  });

export default i18n;